...
desses eventos contra a hipótese nula \cite{moretti2012gcodeml}.
...
conceitos da área de otimização relevantes à compreensão do restante deste
trabalho, com um foco em métodos de busca linear.

Dada uma função $f$, o problema de otimização consiste em encontrar um ponto
$x*$ tal que $f(x*) \le f(x)$ para todo $x$ (otimização global) ou para todo
$x$ próximo de $x*$ (otimização local). Nesse contexto, $f$ é chamada de função
objetivo ou função de custo. A definição supracitada compreende a minimização,
para problemas de maximização basta inverter o sinal de $f$, uma vez que o
mínimo de $f$ será um máximo de $-f$.

Uma técnica clássica de otimização é a pesquisa ou buscar linear, também
conhecida como método de descida. Nela, refina-se iterativamente uma
aproximação $x_i$ de $x*$ movendo um passo de tamanho $\alpha_i$ em uma direção
de descida $p_i$. Isto é, a cada iteração, $x_{i+1} \le x_i$, conforme
expressado na equação de recorrência abaixo.

$$ x_{i + 1} = x_i + \alpha_i p_i $$

Se $f$ é contínua e multivariável, a direção de descida de máximo declive é
$p_i = -B_{i}^{-1} \nabla f_i$, onde $\nabla$ é o gradiente de $f$, i.e. o
vetor de suas derivadas parciais de primeira ordem. Os métodos de busca linear
são classificados pela escolha de $B_i$. Em métodos de primeira ordem como o do
gradiente ou do máximo declive, $B_i$ é a matriz identidade. Em métodos de
segunda ordem como o método de Newton, $B_i = \nabla^2 f(x_i)$. Como métodos de
segunda ordem usam derivadas de segunda ordem, eles exigem que $f$ seja duas
vezes continuamente diferenciável. Enquanto no método de Newton a Hessiana
$\nabla^2$ é exata, em métodos quasi-Newton $\nabla^2$ é aproximado.  Abaixo a
equação de recorrência encontra-se reproduzida na forma usual do método de
Newton.

$$ x_{i + 1} = x_i - \alpha_i \frac{\nabla f_i}{\nabla^2 f_i} $$

Dentro da categoria de métodos de segunda ordem, a forma de aproximação da
Hessiana e a escolha do tamanho do passo definem o método. O passo $\alpha_i$
ideal é dado por outro problema de minimização, $min_\alpha y(\alpha) = f(x_i +
\alpha p_i)$. Métodos que calculam $\alpha_i$ ideal a cada passo são
denominados métodos de busca linear exatos, enquanto aqueles que aproximam
$\alpha_i$ são ditos inexatos. Uma forma de aproximar $\alpha$ de forma inexata
é garantir que ele cumpra uma série de condições, conhecidas como condições de
Wolfe, para um decréscimo suficiente na busca linear.
...
maximização da função de probabilidade, também chamada função de
verossimilhança.
...
gradiente $\nabla$, e as operações de álgebra linear utilizadas na aproximação
da Hessiana $\nabla^2$ e no cálculo de $\alpha$. Os autores observam que os
custos de operações de álgebra linear são normalmente pequenos, o custo do BFGS
sendo dominado pelo custo de avaliação da função. Os autores estimam que, para
...
\includegraphics[width=0.85\linewidth]{img/metodo.png} \end{center}
\legend{Fonte: Os Autores} \label{fig:metodo} \end{figure}
...
paralelas, será medido também seu \textit{speedup}. Feito isso, propõe-se o
...
log-verossimilhança (logaritmo natural da função de verossimilhança) reportado
por cada programa, que é o dado utilizado pelos pesquisadores.
...
descrito na seção \ref{sec:bfgs}. O codeml utiliza ele para maximizar a função
de probabilidade, no escopo da estimativa por máxima verossimilhança. 
...
verossimilhança (ou função de probabilidade) sendo constantemente avaliada
(para atualização do $\alpha$) e diferenciada (para o cálculo do gradiente) no
processo de otimização. Essa rotina, por sua vez, é modularizada em
\textit{fx\_r}. A figura \ref{fig:kcachegrind}, uma captura de tela da
ferramenta kcachegrind, mostra uma visualização da pilha de chamadas
supracitada.
...
estratégia de paralelização nesse \textit{software}, bem como estratégias de
paralelização do BFGS mais complexas que envolvam a avaliação ou diferenciação
da função objetivo. 
...
slimcodeml são da função de probabilidade, cuja avaliação representa o maior
