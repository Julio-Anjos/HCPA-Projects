...
para execução em \textit{cluster}, mais especificamente visando ambientes com o
\textit{middleware} Advanced Resource Connector (ARC), utilizado no \textit{grid} aos
...
\textit{cluster} ARC, portanto essa solução não foi estudada mais a fundo nesse trabalho.
...
ferramentas para compor o pipeline, incluindo a ferramenta sambamba, descrita
...
(quando existentes), a primeira avaliação consiste de um \textit{benchmark} do tempo de
...
Por fim, outro trabalho futuro consiste no estudo de uma abordagem para \textit{cluster}
...
gigabytes para cada entrada. Uma possibilidade é o uso de \textit{frameworks} Apache
