...
execução do \textit{codeml}, e os algoritmos implementados nas rotinas responsáveis pela
...
volátil para isso \cite{immich2003performance}. Num ambiente POSIX, \textit{pipes} são
...
Em suma, o \textit{codeml} detecta eventos de seleção utilizando diferentes modelos de
...
\textit{job} para cada modelo (de variação do $\omega$) do \textit{codeml}, não para
...
bcftools, de etapas de pré e pós processamento dos dados usando o \textit{samtools},
...
na literatura por soluções aos problemas estudados, ou um ciclo de otimização
do software.
...
\ref{sec:test}, geram uma avaliação da necessidade de replanejamento e novo
levantamento de requisitos (iniciando um novo ciclo) ou a entrega da solução
encontrada aos usuários. Esse ciclo PDCA encontra-se reproduzido na parte
direita da Figura \ref{fig:pdca_global}.

\section{Ciclo 3: Otimização}
\label{sec:dev}

Se, no levantamento de requisitos, for avaliado que convém a otimização do software,
um novo ciclo PDCA é iniciado pela arquitetura dessa solução, planejando as
linguagens, bibliotecas e técnicas a serem adotadas.  Em um primeiro momento é
planejado, dentro do universo das soluções possíveis ao problema, aquela que
for de implementação mais simples, para que possa ser rapidamente validado sua
corretude e desempenho. Caso ela seja avaliada como adequada, mas não
suficiente, uma nova iteração do ciclo é iniciada com o planejamento de
...
ciclo de levantamento de requisitos, se conceitos mais básicos precisem ser revistos.
Alternativamente, se a solução empregada é adequada e suficiente, ela é
entregue aos usuários. Esse ciclo PDCA de otimização encontra-se
...
empregada nos ciclos de revisão bibliográfica e otimização. Essas validações
são fundamentais ao trabalho, visto que as soluções adotadas, seja através de
estudo da literatura ou implementação própria, devem produzir resultados
corretos e um desempenho consistentemente satisfatório. Isto é, uma vez que os
métodos empregados na aplicação original e na solução entregue sejam os mesmos,
é necessário, mas não suficiente, validar que os resultados (a saída do
programa) sejam os mesmos, mas também validar que a solução entregue possua
...
pacote \textit{SAMtools}, conforme já exposto no capítulo \ref{chap:anteriores}. Para a
...
desemepenho são aplicados, adicionalmente, testes unitários às funções
...
mensuração do tempo referente às soluções definitivas, são realizadas cinco
execuções, das quais serão reportadas a média e desvio padrão.

Além disso, durante o levantamento de requisitos propõe-se o traço de um perfil de
...
empírico das ferramentas, conforme o ciclo de levantamento de requisitos. Com isso,
...
\textit{codeml} sempre recebe um arquivo de configuração que determina os modelos (de
...
identificadas as aplicações \textit{slimcodeml} e LMAP para validação (de corretude e
...
problemas de desempenho são inerentes à implementação do \textit{codeml} através do
traço de um perfil de execução, a partir do qual avaliou-se que convém iniciar
um ciclo de otimização para eliminar os gargalos de desempenho mapeados,
conforme o ciclo de levantamento de requisitos. Em particular manteve-se em mente a
possibilidade de aplicar técnicas de paralelismo para beneficiar-se dos
ambientes multiprocessados utilizados -- conforme mencionado previamente, o
LMAP e demais implementações paralelizam \textit{jobs} do \textit{codeml}, e
não a aplicação em si. Esse perfil de execução bem como o ciclo de
otimização são descritos na seção \ref{sec:codemlpar}.

\subsection{Validação de aplicações selecionadas: \textit{slimcodeml} e LMAP}
\label{sec:slimeval}

Para a validação de corretude foi comparada a saída da execução do \textit{slimcodeml} e
LMAP com o \textit{codeml} original. Em particular foi comparado o valor da
log-verossimilhança reportado por cada programa. Os modelos de substituição de
sítios do \textit{codeml} utilizados foram o M1a, M2a, M7, M8, e M8a, e os resultados
...
decimal. Trata-se de um artefato de exibição -- o \textit{codeml} original imprime a
saída arredondada à sexta casa decimal, enquanto o \textit{slimcodeml} imprime até a
...
Conforme observado previamente, o \textit{slimcodeml} recebe a mesma entrada e produz a
mesma saída do \textit{codeml} original. Dessa forma, foi testado o LMAP utilizando o
\textit{slimcodeml}, em adição ao LMAP com o \textit{codeml} original, substituído para isso o
executável do \textit{codeml} por aquele do \textit{slimcodeml}. Com isso esperava-se
beneficiar-se da combinação de duas ferramentas que propõem-se a melhorar o
desempenho do \textit{codeml}.
...
de entrada fornecidos por pesquisadores do grupo, com cinco execuções para cada
...
paralela utilizando o LMAP, de até 68\%. Por outro lado, o uso do \textit{slimcodeml}
...
Durante o levantamento de requisitos foi realizado um perfil de execução do \textit{codeml}
...
desempenho observados são inerentes à implementação do \textit{codeml} e
avaliar a otimização através de soluções alternativas às presentes na
literatura (em um ciclo de otimização).
...
diferenciada para o cálculo do gradiente, no processo de otimização (Equação
\ref{eq:mle}). Essa rotina, por sua vez, é modularizada em \textit{fx\_r}. A
Figura \ref{fig:kcachegrind}, uma captura de tela da ferramenta kcachegrind,
apresenta uma visualização da pilha de chamadas supracitada.
...
identificados, iniciando um ciclo de otimização, descrito abaixo.
...
% TODO encaixar melhor em outro lugar
Por fim, observa-se que algumas das operações matriciais otimizadas pelo
\textit{slimcodeml} são da função de verossimilhança, cuja avaliação representa o maior
custo do \textit{codeml}. Por exemplo, sua sub-rotina \textit{PMatUVRoot}, que pode ser
visualizada na Figura \ref{fig:kcachegrind}, tem sua implementação substituída
por aquela da biblioteca BLAS -- no \textit{slimcodeml}, ela é renomeada para
\textit{PMatUVRoot2}.
...
Já a aplicação bcftools, parte do pacote \textit{SAMtools}, providencia comandos para
...
equivalentes da ferramenta \textit{samtools}.  Primeiramente, foi validado que o
conteúdo dos arquivos produzidos é o mesmo, então, foi validado seu desempenho,
através do tempo médio de cinco execuções. A amostra utilizada foi a HGDP00519,
...
\subsection{\textit{SAMtools}: Perfil de execução e paralelização}
...
por motivos históricos, pois originalmente o \textit{SAMtools} não trabalhava com
...
Assim, iniciou-se um ciclo de otimização, que será descrito nesta seção.
...
expostas validações da solução completa. Todavia, convém mencionar que a
otimização se deu por etapas, já discriminadas acima, de acordo com a
...
foram replicadas cinco vezes. Na Tabela \ref{tbl:SAMtools} encontram-se
reproduzidos os tempos de execução utilizando o \textit{pipeline} original (uma
execução) e o otimizado (média e desvio de cinco execuções). Observa-se que a
...
\begin{table}[h]
    \caption{Tempos de execução do \textit{SAMtools} \textit{versus} a ferramenta desenvolvida}
    \centering
...
Tanto o \textit{samtools} como o bcftools e a ferramenta desenvolvida pelos autores
...
interface gráfica para o uso do \textit{samtools} e bcftools para chamada de variantes.
...
para uso do angsd. Essa conversão é realizada utilizando o software
...
análise feita com o angsd (PBS) utilizando o pacote \textit{SAMtools}, construído em
...
descrito um estudo desse \textit{pipeline} e posterior otimização através do
...
ferramental, a análise pelo angsd sendo substituída pela análise pelo
\textit{SAMtools} por parte dos pesquisadores.
...
\textit{sambamba} e \textit{elPrep}, a fim de adequá-las aos arquivos atualmente disponibilizados
