...
reduzindo de dias para minutos o tempo de processamento explorando o fato de
diversas etapas dele serem embaraçosamente paralelas - paralelizei-as com GNU
Parallel, sem paralelizar as aplicações - além de remover etapas custosas mas
desnecessárias, identificadas através do estudo da literatura e experimentação.

Em reuniões com a Agnis compreendi que existem dois pipelines que podem ser
utilizados, usando o software angsd ou o software samtools, ambos usando a
...
Desenvolvi scripts de execução das etapas para comparar o desempenho do
piepline original e do paralelizado e reduzido, bem como os resultados de ambos
a fim de garantir a corretude do pipeline otimizado. Ambos apresentaram os
mesmos resultados, mas com tempos de execução significativamente melhores no
pipeline otimizado.

Como próximos passos pretendo discutir com a Agnis os resultados e seguir
explorando as etapas restantes da análise. Acredito que possa ser desenvolvida
uma ferramenta que execute o pipeline de ponta a ponta explorando do
paralelismo das etapas sempre que possível, facilitando e acelerante a análise
filogenética.
...
podem ser realizadas em paralelo. Aĺém disso, a etapa de view da região pode
ser realizada em paralelo para cada região.

Com isso em mente, obtive um segundo CRAM e implementei um script para realizar as etapas em paralelo com o GNU Parallel:
